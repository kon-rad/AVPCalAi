...
class ViewModel: NSObject, AVAudioRecorderDelegate, AVAudioPlayerDelegate {
    ...
    let client = OpenAIClient(apiKey: "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")
    ...
    func processSpeechTask(audioData: Data) -> Task<Void, Never> {
        Task { @MainActor [unowned self] in
            do {
                ...
                var prompt = try await client.generateAudioTransciptions(audioData: audioData)
                print("prompt: ", prompt)
                prompt = "You are an Ai calandar assistant in professional space, give the appropriate response to this question, be helpful, give advice. Use Calendar data like this: Today I have a Hackathon at HF0, from 11:30 to 3PM. Question: " + prompt
